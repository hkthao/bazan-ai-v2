...
def extract_pdf_ocr(path: Path) -> ExtractedDoc:
    ...
    try:
        ...
        for i, page in enumerate(doc):
            ...
            result = run_ocr(
                [img], [["vi", "en"]], det_model, det_processor, rec_model, rec_processor
            )
            page_text = " ".join([line.text for line in result[0].text_lines])
            full_text += f"\n\n[Trang {i + 1} - OCR]\n{page_text}"
            sections.append(
                {"heading": f"Trang {i + 1}", "level": 1, "content": page_text, "page": i + 1}
            )

        return ExtractedDoc(
            text=full_text, sections=sections, page_count=len(doc), extraction_method="ocr"
        )
    except ImportError:
        # surya-ocr không có — trả về empty với warning
        print(f"  WARN: surya-ocr chưa cài, không thể OCR {path.name}")
        return ExtractedDoc(text="", sections=[], page_count=0, extraction_method="ocr_unavailable")
